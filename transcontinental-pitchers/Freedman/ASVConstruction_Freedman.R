...
primer <- "16S"
wd <- "Freedman/fastq/demultiplexing"
...
sample.names <- paste(substr(basename(rF), 0, 5),
                      gsub("001.", "",
                           gsub(".fastq", "",
                                sapply(strsplit(basename(rF), "_"), `[`, 5))),
                      sep = "_")
...
rF.fN <- sort(list.files(path = paste0(wd, "/filtN"), pattern = "R1", full.names = TRUE))
rR.fN <- sort(list.files(path = paste0(wd, "/filtN"), pattern = "R2", full.names = TRUE))
...
# saveRDS(seqtab.nochim, "seqtab.nochim_ZF.rds")
