...
for (i in 1:length(wd)) {
  ...
  ## Grab the sequences and taxonomy file
  # setwd(paste0("./", wd[i]))
  rds <- list.files(path = wd[i],
                    pattern = "seqtab|taxa_")
  # print(wd[i])
  print(rds)
  seqtab.nochim <- readRDS(paste(wd[i], rds[1], sep = "/"))
  taxa.id <- readRDS(paste(wd[i], rds[2], sep = "/"))
