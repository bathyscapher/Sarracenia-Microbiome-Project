...
vascu <- read.table("~/Seafile//PhDFribourg/GIS/Ellenberg/VascularPlants_InfoSpecies.csv",
...
bryo <- read.table("~/Seafile/PhDFribourg/GIS/Ellenberg/Bryophyta_InfoSpecies.csv",
                    sep = "\t", header = TRUE, quote = "")

bryo2 <- read.table("~/Seafile/_FNS_2016-NicheDynamCoexist/SarraceniaMicrobiomeProject/Mosses/SMP_Bryophyta.csv",
                   sep = "\t", header = TRUE, quote = "")
...
# levels(as.factor(plants$Taxon)) == levels(as.factor(test$Taxon))
...
## Delete f.
plants$SpeciesShort <- gsub("f\\.", "", plants$SpeciesShort, perl = TRUE)
...
write.table(ev.mean,
            "~/Sarracenia-Microbiome-Project/Thesis/csv/Moss_EVMeans.csv",
            sep = "\t", row.names = FALSE)
